...
function renderArticles() {
  ...
  // Mélanger : articles CMS en tête + articles de démo
...
const articles = [
  {
    category: 'Politique',
    title: 'Sommet régional : les dirigeants africains tracent la feuille de route 2030',
    excerpt: 'Une réunion historique pour définir les grandes orientations de développement du continent.',
    date: '21 Avr 2025',
    icon: 'fa-landmark'
  },
  {
    category: 'Sport',
    title: 'Finale nationale de football : Saint-Louis en liesse après la victoire',
    excerpt: 'Un match intense qui restera dans les annales du football sénégalais.',
    date: '20 Avr 2025',
    icon: 'fa-futbol'
  },
  {
    category: 'Santé',
    title: 'Campagne nationale de vaccination : des milliers de personnes mobilisées',
    excerpt: 'Les équipes médicales parcourent les zones rurales pour atteindre les communautés isolées.',
    date: '19 Avr 2025',
    icon: 'fa-syringe'
  },
  {
    category: 'Économie',
    title: 'Croissance du marché local : les entrepreneurs de Dagana voient grand',
    excerpt: 'Le tissu économique régional se diversifie grâce aux initiatives des jeunes entrepreneurs.',
    date: '18 Avr 2025',
    icon: 'fa-chart-line'
  }
];
...
const societéArticles = [
  {
    category: 'Société',
    title: 'Éducation : l\'inauguration de trois nouvelles salles de classe à Gaé',
    date: '20 Avr 2025',
    icon: 'fa-graduation-cap'
  },
  {
    category: 'Société',
    title: 'Solidarité : la communauté se mobilise pour les familles sinistrées',
    date: '19 Avr 2025',
    icon: 'fa-hands-holding-heart'
  },
  {
    category: 'Société',
    title: 'Agriculture : la saison des pluies s\'annonce prometteuse pour la vallée du fleuve',
    date: '17 Avr 2025',
    icon: 'fa-seedling'
  }
];
...
function renderArticles() {
  ...
  grid.innerHTML = articles.map(a => `
    <article class="article-card">
      <div class="article-card-img">
        <span class="placeholder-icon"><i class="fa-solid ${a.icon}"></i></span>
      </div>
      <div class="article-card-body">
  ...
  list.innerHTML = societéArticles.map(a => `
    <div class="article-list-item">
      <div class="article-list-thumb">
        <span class="ph"><i class="fa-solid ${a.icon}"></i></span>
      </div>
...
function renderPopular() {
  ...
  list.innerHTML = popularArticles.map(title => `
...
function subscribeNewsletter(e) {
  ...
  if (msgEl) {
    ...
    msgEl.textContent = '✓ Merci ! Vous recevrez nos actualités par e-mail.';
    msgEl.style.color = '#1a7c4f';
  }
...
const CHANNEL_ID = "UCZjbs0MGrC3IeKfjuuO1BgA"; // GayaInfo TV
    ...
    if (data.items && data.items.length > 0) {
      ...
      container.innerHTML = `
        <a href="https://www.youtube.com/watch?v=${videoId}"
           target="_blank" class="live-link-btn">
          <i class="fa-brands fa-youtube"></i> Regarder le live en direct
        </a>`;
      titleEl.textContent = title;

    } else {
      container.innerHTML = `
        <p style="text-align:center; padding:1.5rem; color:#aaa;">
          <i class="fa-solid fa-circle-xmark"></i><br>
          Aucun live en ce moment.<br>Revenez plus tard !
        </p>`;
      titleEl.textContent = "Pas de live pour le moment";
    }

  } catch(e) {
    document.getElementById("live-content").innerHTML =
      `<p style="text-align:center; color:#aaa; padding:1rem;">Impossible de vérifier le live.</p>`;
  }
}
...
function showToast(event, iconClass) {
  ...
  const toastW = 260; // largeur estimée
  const margin = 12;

  // Éviter de déborder à droite
  const left = (x + toastW + margin > window.innerWidth)
    ? x - toastW - margin
    : x + margin;
  ...
  // Origine de l'animation depuis le coin proche du clic
...
// Bouton Radio (header)
document.querySelector('.btn-radio').addEventListener('click', function(e) {
  e.preventDefault();
...
document.querySelectorAll('.footer-col a[href="#"]').forEach(link => {
  link.addEventListener('click', function(e) {
    e.preventDefault();
    const icon = this.textContent.trim().includes('Sport')
      ? 'fa-solid fa-futbol'
      : 'fa-solid fa-radio';
    showToast(e, icon);
  });
